...
raw_wine_reviews.df <- read.csv("wine_reviews.csv")
...
wine_reviews.df <-  raw_wine_reviews.df %>%
  ...
  dplyr::select(
    points, price, year, red, dom, taster_following, el, lat, lon, p88, taster_name
  ) %>% 
  filter_all(function(x){!is.na(x)})
...
wine <- winedata %>% 
 ...
 #    dom = ifelse(country == "US", 1, 0)
 #  ) %>%
 dplyr::select(points, price, year, taster_following, el, lat, lon) %>% 
 mutate_all(as.numeric) %>% 
 filter_all(function(x){!is.na(x)}) 
