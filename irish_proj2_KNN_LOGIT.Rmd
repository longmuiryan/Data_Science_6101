...
```{r subset features}
wine2 <- subset(wine_reviews, select = c(country, points, price, province, variety, winery, year, color))
...
wine2$color<- as.numeric(wine2$color)
...
scaled_wine2 <- as.data.frame(scale(wine2[1:7], center = TRUE, scale = TRUE))

set.seed(1000)
wine2_sample <- sample(2, nrow(scaled_wine2), replace=TRUE, prob=c(0.75, 0.25))

wine2_training <- scaled_wine2[wine2_sample==1, 1:7]
wine2_test <- scaled_wine2[wine2_sample==2, 1:7]
```
```{r Y-labels}

wine2.trainLabels <- wine2[wine2_sample==1, 8]
wine2.testLabels <- wine2[wine2_sample==2, 8]
```
```{r KNN model}
...
logit_wine2 <- glm(points ~ ., data = wine2, binomial(link = "logit"))
summary(logit_wine2)
exp(coef(logit_wine2))

confint(logit_wine2)
confint.default(logit_wine2)
