...
```{r Barplots of Wine Production, fig.align = "center", fig.cap="**Figure 1:** (a) Wine production by country (b) Wine production by state within the United States."}
...
```{r France, fig.align = "center", fig.cap="**Figure 2**: Geographic distribution of wine reviewed from France."}
# France
...
```{r Italy, fig.align = "center", fig.cap="**Figure 3:** Geographic distribution of wine reviewed from Italy"}
# Italy
...
```{r California, fig.align = "center", fig.cap="**Figure 4:** Geographic distribution of wine reviewed from California."}
...
```{r Normality I, fig.align = "center", fig.cap="**Figure 5:** Year (a) histogram, (b) QQ plot, and (c) box plot."}
# Box Plot 
box <- wine_reviews %>%
...
```
  ...
  Upon removing outliers, we learned that the majority of our reviews occured between 2004 and 2016 with a median year of 2012 and mean of 2011 (Fig 5). The important thing to keep in mind here is that while the dataset provides a recorded year in the observation, it simply means that the wine was reviewed in that year and not necessarily made that year.  
  
```{r Normality II, fig.align = "center", fig.cap="**Figure 6:** Price (a) histogram, (b) QQ plot, and (c) box plot."}
# Box Plot 
box <- wine_reviews %>%
...
wine_reviews %>% 
  ...
  labs(title = "Point Distribution by Region", x = "Points", y = "Region") + 
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5))
```

  Due to the level of analysis required to constructively analyze the impact of price and quality (points) on wines from Old World and the New World, a more granular inspection is required. To best analyze the point distribution, the following price bins were created: >\$15, \$15 to \$29, \$30 to \$44, \$45 to \$60, and >\$60 (Fig 9). Seemingly, the results suggest a positive correlation between cost and quality points for all regions investigated.  
...
```{r, fig.align = "center", fig.cap="**Figure 9a:** Boxplot of point distribution by price for California."}
# box plot by bucket 
ca <- wine_reviews %>% 
...
```{r, fig.align = "center", fig.cap="**Figure 9b:** Boxplot of point distribution by price for Italy"}
it <- wine_reviews %>% 
  filter(region == "Italy") %>% 
...
```{r, fig.align = "center", fig.cap="**Figure 9c:** Boxplot of point distribution by price for France"}
fr <- wine_reviews %>% 
...
```{r Overlayed Histagram, fig.align = "center", fig.cap="**Figure 10:** Density plot overlayed with point distribition from each region."}
# Renamed variables 
wine_reviews <- wine_reviews %>% 
  mutate(region  = case_when(
    province == "California" ~ "California",
...
summary(model) %>% 
  xtable() %>% 
  kable(caption = "Table 1: Regression results.", digits = 3) %>%
...
Agafonkin, V. (2019, November 17). an open-source JavaScript library for interactive maps. Retrieved March 25, 2020, from [https://leafletjs.com/](https://leafletjs.com/)

Karlsson, P., & Karlsson, B. (2019, April 14). Record Global Wine Harvest In 2018, Stable Consumption. Retrieved March 25, 2020,  from [https://www.forbes.com/sites/karlsson/2019/04/14/record-global-wine-harvest-in-2018-stable-consumption/#7c7af2d4266b](https://www.forbes.com/sites/karlsson/2019/04/14/record-global-wine-harvest-in-2018-stable-consumption/#7c7af2d4266b)

Robinson, J., & Harding, J. (2006). The Oxford Companion to Wine (3rd ed.). Oxford University Press.

Leaflet for R. (n.d.). Retrieved March 25, 2020, from [http://rstudio.github.io/leaflet/](http://rstudio.github.io/leaflet/)

Lombardo, C. (2012). Retrieved March 25, 2020, from [https://pdfs.semanticscholar.org/85b9/4cfbc70b6e10c659a79f48d877974bddb11a.pdf](https://pdfs.semanticscholar.org/85b9/4cfbc70b6e10c659a79f48d877974bddb11a.pdf)

McCarthy, E., & Ewing-Mulligan, M. (2019). Wine for Dummies (7th ed.). Retrieved March 25, 2020, from [www.wiley.com](www.wiley.com)

Montgomery, K. (n.d.). Wine Preference is Individual, Not Male or Female. Wine Magazine by Zzyh. Retrieved March 25, 2020, from [https://us.zzysh.me/blog/wine-preferences-not-male-female/](https://us.zzysh.me/blog/wine-preferences-not-male-female/)
...
Zackthoutt. (2017, November 27). Wine Reviews. Retrieved March 25, 2020, from [https://www.kaggle.com/zynicide/wine-reviews](https://www.kaggle.com/zynicide/wine-reviews)

## Supplemental Figures

```{r, fig.align = "center", fig.cap="**Figure S1:** A scatter plot of price vs. points."}
...
```{r, fig.align = "center", fig.cap="**Figure S2:** An animated visual illustrating the number of reviews in each region per year."}
wine_reviews %>% 
  filter(!is.na(region)) %>% 
  ...
  labs(title = "Year: {frame_time}", x = "Region", y = "Count") +
  theme_minimal() + theme(plot.title = element_text(hjust = 0.5)) +
